...
func (res EventManaget) GetEvent(clientName, key string, target interface{}) (eventReleaseIn time.Duration, err error) {
	...
	dbAddress, err := res.DBIndex.Get(ctx, helpers.GenerateKeyIndex(clientName, key)).Int()
	if err != nil {
...
func (res EventManaget) UpdateExpiredEvent(clientName, key string, expired time.Duration) error {
	...
	if err := res.DBIndex.Set(ctx, helpers.GenerateKeyIndex(clientName, key), dbAddress, expired).Err(); err != nil {
		return err
	}
...
func (res EventManaget) SetEventReleaseAt(clientName, key string, releaseAt time.Time, event models.Event) error {
	ctx := context.Background()
	data, err := json.Marshal(event)
	if err != nil {
		return err
	...
	if err := res.DBIndex.Set(ctx, helpers.GenerateKeyIndex(clientName, key), res.DBPointer, -1).Err(); err != nil {
		return err
	}

	if err := res.DBIndex.ExpireAt(ctx, helpers.GenerateKeyIndex(clientName, key), releaseAt).Err(); err != nil {
		return err
	}

	if err := res.DB[res.DBPointer].Set(ctx, helpers.GenerateKeyEvent(clientName, key), "event-key", -1).Err(); err != nil {
		return err
	}

	if err := res.DB[res.DBPointer].ExpireAt(ctx, helpers.GenerateKeyEvent(clientName, key), releaseAt).Err(); err != nil {
		return err
	}
...
func (res EventManaget) UpdateEventReleaseAt(clientName, key string, releaseAt time.Time) error {
	ctx := context.Background()
	dbAddress, err := res.DBIndex.Get(ctx, helpers.GenerateKeyIndex(clientName, key)).Int()
	if err != nil {
		return fmt.Errorf("data not found")
	...
	if err := res.DB[dbAddress].ExpireAt(ctx, helpers.GenerateKeyEvent(clientName, key), releaseAt).Err(); err != nil {
		return err
	}
